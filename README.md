...
一个基于 Flutter 开发的专用 AI 助手客户端，支持 OpenAI 通用兼容格式的流式聊天、视觉分析、AI 图片生成和现代化沉浸式 UI。
...
- **现代 UI**：深色渐变背景、玻璃态面板、快捷功能卡片、悬浮输入框和新版消息气泡。
...
- 生图模型自动清洗：如果误填 `gpt-40`、`gpt-4o`、`gpt-4o-mini` 等聊天模型，会自动回退为 `dall-e-3`。
...
说明 `gpt-40` 被服务商当作了请求模型。`gpt-40` 通常是把字母 `o` 误写成数字 `0`。新版本中：
