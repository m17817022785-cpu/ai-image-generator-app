...
class ToolDecision {
  ...
  bool get shouldCallImageTool =>
      action == ToolAction.textToImage || action == ToolAction.imageToImage;
}
...
class ApiService {
  ...
  Uri _endpoint(String baseUrl, String path) {
    ...
    final isChat = currentPath == '/chat' ||
        currentPath.endsWith('/chat/completions') ||
        currentPath.endsWith('/completions');
    ...
    if (path == '/images/generations' && isGeneration)
      return Uri.parse(normalized);
    if (path == '/images/edits' && isEdit) return Uri.parse(normalized);
    if ((path == '/images/generations' || path == '/images/edits') && isChat) {
      return Uri.parse('${_stripKnownEndpointSuffix(normalized)}$path');
    }
    return Uri.parse('$normalized$path');
  }

  String _stripKnownEndpointSuffix(String url) {
    var value = url;
  ...
  bool _useChatImageEndpoint(String baseUrl) {
    final path =
        Uri.tryParse(_normalizeBaseUrl(baseUrl))?.path.toLowerCase() ?? '';
    return path == '/chat';
  }

  Future<List<String>> fetchModels(
      {required String apiKey, required String baseUrl}) async {
    final url = _modelsEndpoint(baseUrl);
    _log.info('models', '开始获取服务商模型', 'GET /models',
    ...
    if (response.statusCode < 200 || response.statusCode >= 300)
      throw Exception(
          '获取模型失败 ${response.statusCode}: ${_extractError(bodyText)}');
    final parsed = _decodeJsonObject(bodyText,
        category: 'models',
        title: '获取服务商模型响应解析失败',
    ...
    _log.success('models', '服务商模型获取完成', '${unique.length} models',
        details: {'endpoint': url.toString(), 'models': unique});
    return unique;
  }
  ...
  Map<String, dynamic> _decodeJsonObject(String body,
    ...
    if (_looksLikeHtml(body)) {
      ...
      throw Exception(
          '接口返回了 HTML 页面，不是 OpenAI 兼容 JSON。请检查 Base URL。Endpoint: $endpoint');
    }
    try {
  ...
  String _extractError(String body) {
    try {
      ...
      if (error is Map) {
        ...
        if (message != null && message.isNotEmpty) {
          ...
          if (lower.contains('auth_not_found') ||
    ...
    if (_looksLikeHtml(body))
      return '接口返回 HTML 页面，不是 JSON。响应预览：${_shortBody(body, maxLength: 300)}';
    return body;
  }

  String _friendlyImageError(String raw, String requestedModel,
      String actualModel, String endpointName) {
    final msg = _extractError(raw);
    final lower = msg.toLowerCase();
    if (lower.contains('model not found') ||
        lower.contains('no available channel'))
      return '$endpointName 模型不可用。当前实际请求模型是：$actualModel。请填写服务商支持的图片模型。原始错误：$msg';
    if (lower.contains('not found') ||
        lower.contains('404') ||
        lower.contains('no route'))
      return '$endpointName 接口不可用。请确认图片 Base URL 是否支持对应接口。原始错误：$msg';
    if (requestedModel.trim() != actualModel)
      return '检测到模型配置不适合图片工具，已从“$requestedModel”自动改用“$actualModel”，但接口仍返回错误：$msg';
    return msg;
  }

  List<String> _effectiveImages(
      String? base64Image, List<String>? base64Images) {
    final images = <String>[];
    if (base64Images != null)
      images
          .addAll(base64Images.map((e) => e.trim()).where((e) => e.isNotEmpty));
    if (images.isEmpty && base64Image != null && base64Image.trim().isNotEmpty)
      images.add(base64Image.trim());
    return images;
  }

  Map<String, dynamic> _imageContentMessage(
      String role, String text, String? base64Image,
...
规则：\n1. 图片分析、图片内容描述、看图问答、识别画面、询问建议、写文案、让你解释图片时，必须使用 direct_answer，即使用户上传了图片也不要调用 image_to_image。\n2. 只有用户明确要求生成新图、画一张图、出图、重绘、换风格、修复、编辑、把参考图做成新画面时，才调用图片工具。\n3. 无上传图且明确要生成图片时使用 text_to_image。\n4. 有上传图且明确要求参考/修改/重绘/换风格/修复/生成新图时使用 image_to_image。注意：参考图只是内容/风格参考，size 表示返回图片画幅。\n5. 没有上传图不要 image_to_image。\nsize 默认 1024x1024，横图可 1792x1024，竖图可 1024x1792。quality 默认 auto，高清可 hd/high。''';
    _log.info('tool_decision', '开始 LLM 工具决策', 'POST /chat/completions',
        details: {
          'endpoint': url.toString(),
          'model': actualModel,
          'hasImage': hasImage,
    ...
    final response = await http.post(url,
        ...
        body: jsonEncode({
          ...
          'messages': [
            ...
            _imageContentMessage(
                'user', userText.isEmpty ? '请根据上传图片继续处理' : userText, null,
                base64Images: images)
          ],
          'stream': false
        }));
    ...
    if (response.statusCode < 200 || response.statusCode >= 300)
      throw Exception(
          '工具决策失败 ${response.statusCode}: ${_extractError(bodyText)}');
    final parsed = _decodeJsonObject(bodyText,
    ...
    final decision =
        _parseDecision(raw, userText: userText, hasImage: hasImage);
  ...
  ToolDecision _parseDecision(String raw,
      {required String userText, required bool hasImage}) {
    ...
    try {
      ...
      final action = switch (text) {
        'text_to_image' => ToolAction.textToImage,
        'image_to_image' =>
          hasImage ? ToolAction.imageToImage : ToolAction.textToImage,
        _ => ToolAction.directAnswer
      };
      final prompt = data['prompt']?.toString().trim() ?? userText.trim();
      final reply = data['reply']?.toString().trim() ?? '';
      final size = data['size']?.toString().trim().isNotEmpty == true
          ? data['size'].toString().trim()
          : '1024x1024';
      final quality = data['quality']?.toString().trim().isNotEmpty == true
          ? data['quality'].toString().trim()
          : 'auto';
      return ToolDecision(
          action: action,
          prompt: prompt.isEmpty ? userText : prompt,
          reply: reply,
          size: size,
          quality: quality);
    } catch (e) {
      _log.warning('tool_decision', 'LLM 决策 JSON 解析失败', '使用兜底逻辑',
          details: {'rawContent': raw, 'error': e.toString()});
      if (!hasImage && _looksLikeImagePrompt(userText))
        return ToolDecision(
            action: ToolAction.textToImage,
            prompt: userText,
            reply: '我会为你生成图片。',
            size: '1024x1024');
      return ToolDecision(
          action: ToolAction.directAnswer,
          prompt: '',
          reply: raw.trim().isEmpty ? '我暂时无法判断你的意图，请换个说法。' : raw.trim(),
          size: '1024x1024');
    }
  }
  ...
  String? _normalizeImageQuality(String quality, String model) {
    ...
    if (lowerModel.startsWith('dall-e-3'))
      return value == 'hd' || value == 'high' ? 'hd' : 'standard';
    ...
    if (value == 'hd' ||
        ...
        value == 'low' ||
        value == 'medium' ||
        value == 'high') return value;
    return null;
  }

  String _normalizeImageSize(String size, String model) {
    final lowerModel = model.toLowerCase();
    if (!lowerModel.contains('gpt-image')) return size;
    return switch (size) {
      '1792x1024' || '1024x768' => '1536x1024',
      '1024x1792' || '768x1024' => '1024x1536',
      '1024x1024' || '1536x1024' || '1024x1536' || 'auto' => size,
      _ => '1024x1024',
    };
  }

  bool _supportsInputFidelity(String model) {
    final lower = model.toLowerCase();
    return lower == 'gpt-image-1' || lower.startsWith('gpt-image-1-');
  }
  ...
  Future<String> refineImagePrompt(
      {required String userText,
      required String? base64Image,
      List<String>? base64Images,
      required String apiKey,
...
如果用户输入已经完整，只做结构优化、冲突清理和稳定性增强。''';
    ...
    final response = await http.post(url,
        ...
        body: jsonEncode({
          ...
          'messages': [
            {'role': 'system', 'content': systemPrompt},
            _imageContentMessage('user', userInstruction, null,
                base64Images: _effectiveImages(base64Image, base64Images))
          ],
          'stream': false
        }));
    final bodyText = utf8.decode(response.bodyBytes);
    if (response.statusCode < 200 || response.statusCode >= 300)
      throw Exception(
          '提示词润色失败 ${response.statusCode}: ${_extractError(bodyText)}');
    final parsed = _decodeJsonObject(bodyText,
        category: 'prompt_refine',
        title: 'LLM 润色提示词响应解析失败',
        endpoint: url.toString(),
        model: actualModel,
        statusCode: response.statusCode);
    final refined =
        parsed['choices']?[0]?['message']?['content']?.toString().trim() ?? '';
    return _neutralizeAutoAddedIdentityTerms(
        refined.isEmpty ? userText : refined,
        originalText: userText,
        hasReferenceImage:
            _effectiveImages(base64Image, base64Images).isNotEmpty);
  }
  ...
  String _neutralizeAutoAddedIdentityTerms(String prompt,
      {required String originalText, required bool hasReferenceImage}) {
    if (!hasReferenceImage || _originalMentionsIdentityOrAge(originalText))
      return prompt.trim();
    var value = prompt;
    final replacements = <RegExp, String>{
      RegExp(r'\byoung\s+woman\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\byoung\s+girl\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\badult\s+woman\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\badult\s+man\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\bteen(?:ager)?\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\bgirl\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\bboy\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\bwoman\b', caseSensitive: false):
          'the person in the reference image',
      RegExp(r'\bman\b', caseSensitive: false):
          'the person in the reference image',
  ...
  Stream<String> generateChatStream(List<Message> history, String apiKey,
    ...
    final request = http.Request('POST', url)
      ...
      ..body = jsonEncode({
        'model': actualModel,
        'messages': history.map((msg) => msg.toOpenAiMap()).toList(),
        'stream': true
      });
    ...
    try {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        ...
        throw Exception(
            'API 错误码 ${response.statusCode}: ${_extractError(errorMsg)}');
      }
  ...
  Future<String> generateImage(
      String prompt, String apiKey, String baseUrl, String model,
      {String size = '1024x1024', String quality = 'auto'}) async {
    ...
    final requestSize = _normalizeImageSize(size, imageModel);
    ...
    return _extractImageResult(bodyText,
        endpoint: url.toString(), model: imageModel, category: 'text_to_image');
  }

  Future<String> editImage(String prompt, File imageFile, String apiKey,
    ...
    final requestSize = _normalizeImageSize(size, editModel);
    ...
    if (_supportsInputFidelity(editModel))
      request.fields['input_fidelity'] = 'high';
    request.files
    ...
    if (response.statusCode < 200 || response.statusCode >= 300)
      throw Exception(
          _friendlyImageError(bodyText, model, editModel, '图生图 / 图片编辑'));
    return _extractImageResult(bodyText,
        endpoint: url.toString(), model: editModel, category: 'image_to_image');
  }

  Future<String> editImages(String prompt, List<File> imageFiles, String apiKey,
      String baseUrl, String model,
      {String size = '1024x1024', String quality = 'auto'}) async {
    if (imageFiles.isEmpty)
      return generateImage(prompt, apiKey, baseUrl, model,
          size: size, quality: quality);
    if (imageFiles.length == 1)
      return editImage(prompt, imageFiles.first, apiKey, baseUrl, model,
          size: size, quality: quality);
    if (_useChatImageEndpoint(baseUrl)) {
      final images = <String>[];
  ...
  Future<String> _callChatStyleImageTool(
    ...
    final url = _endpoint(baseUrl, '/chat/completions');
    final actualModel =
        model.trim().isEmpty ? normalizeChatModel(model) : model.trim();
    final images = _effectiveImages(base64Image, base64Images);
    final instruction = images.isNotEmpty
        ? '$prompt\n\n必须严格参考上传图片的主体、构图、人物特征、服装、姿态和关键视觉元素进行生成/编辑，不要忽略参考图。返回图片 URL 或 base64 图片数据。尺寸：$size。清晰度/质量：${_qualityLabel(quality)}。'
          ...
          'messages': [
            {
              'role': 'system',
              'content':
                  '你是图片生成/编辑工具。请只返回最终图片 URL 或 data:image/...;base64,...，不要输出多余解释。'
            },
            _imageContentMessage('user', instruction, null,
                base64Images: images)
          ],
          'stream': false
        }));
    ...
    return _extractImageResultFlexible(bodyText,
        endpoint: url.toString(), model: actualModel, category: category);
  }

  String _extractImageResultFlexible(String bodyText,
      {required String endpoint,
      required String model,
      required String category}) {
    final data = _decodeJsonObject(bodyText,
        category: category,
        title: '图片接口响应解析失败',
        endpoint: endpoint,
        model: model);
    ...
    throw Exception(
        '图片接口未返回图片 URL 或 base64。Endpoint: $endpoint。响应预览：${_shortBody(bodyText)}');
  }
  ...
  String? _tryExtractImageFromText(String text) {
    ...
    final dataMatch =
        RegExp(r'data:image/[a-zA-Z0-9.+-]+;base64,[A-Za-z0-9+/=\r\n]+')
            .firstMatch(trimmed);
    if (dataMatch != null)
      return dataMatch.group(0)!.replaceAll(RegExp(r'\s+'), '');
    final markdownImage =
